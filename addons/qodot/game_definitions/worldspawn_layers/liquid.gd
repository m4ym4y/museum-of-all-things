...
func _physics_process(delta: float) -> void:
	for body in buoyancy_dict:
		...
		body.apply_central_force(Vector3.UP * displacement * buoyancy_factor)
		body.apply_central_force(Vector3(1, 0, 1) * body.get_linear_velocity() * displacement * -lateral_damping_factor)
		body.apply_central_force(Vector3(0, 1, 0) * body.get_linear_velocity() * -vertical_damping_factor)
